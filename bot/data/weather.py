...
def get_weather_hour():
    url = f"https://api.weatherapi.com/v1/forecast.json?key=cd561d4bc365492682c45737242007&q=40.3833,71.7833&days=7"
    response = requests.get(url).json()['forecast']['forecastday']
    result = "1 soatlik ma'lumotlar:\n"
    for i in response:
        date = i['date']
        maxtemp_c = i['hour']['maxtemp_c']
        mintemp_c = i['hour']['mintemp_c']
        avgtemp_c = i['hour']['avgtemp_c']
        maxwind_kph = i['hour']['maxwind_kph']
        avghumidity = i['hour']['avghumidity']
